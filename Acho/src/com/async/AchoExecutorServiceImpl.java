...
public class AchoExecutorServiceImpl implements AchoExecutorService {
	...
	static final class AchoTaskWrapper implements Runnable {

		private AchoTask task;
	...
	static final class AchoThreadPool extends ThreadPoolExecutor {
		 ...
		 * garbage collected
		 **/
		private final Map<AchoTaskCallback, List<AchoTaskWrapper>> pendingTasks;

		private final CleanupThread cleanupThread;

		/** map to help us not to schedule duplicate petitions **/
		private final Map<String, List<AchoTask>> dedupingTaskList;
		 ...
		 */
		public AchoThreadPool(ReferenceQueue<AchoTaskCallback> referenceQueue) {
			super(DEFAULT_THREAD_COUNT, DEFAULT_THREAD_COUNT, 0, TimeUnit.MILLISECONDS,
					new LinkedBlockingQueue<Runnable>(DEFAULT_QUEUE_CAPACITY), new AchoThreadFactory());
			...
			this.pendingTasks = Collections.synchronizedMap(new WeakHashMap<AchoTaskCallback, List<AchoTaskWrapper>>());
			this.dedupingTaskList = new ConcurrentHashMap<String, List<AchoTask>>();
		...
		public void execute(AchoTask task) {
			...
			AchoTaskWrapper wrapper = new AchoTaskWrapper(task);

			List<AchoTaskWrapper> wrapperList;
			List<AchoTask> taskList;
			
			synchronized(this) {
				//first pending tasks
				if (this.pendingTasks.get(task.getCallback()) == null) {
					wrapperList = new ArrayList<AchoTaskWrapper>();
				} else {
				...
				if (this.dedupingTaskList.get(task.getTaskId())==null) {
					taskList = new ArrayList<AchoTask>();
				} else {
		 ...
		 * 
		 */
		private boolean isDuplicateTask(AchoTask task) {
			
			if (this.dedupingTaskList.get(task.getTaskId()) != null) {
				List<AchoTask> similarTasks = this.dedupingTaskList.get(task.getTaskId());
				// check all entries of this tasks to see if one is equal
				// to the one we want to schedule, if so return
				Iterator<AchoTask> it = similarTasks.iterator();
		 ...
		 * @param callback
		 */
		private void removeTask(AchoTaskCallback callback) {
			List<AchoTaskWrapper> pending = this.pendingTasks.get(callback);
			this.pendingTasks.remove(callback);

			if (!pending.isEmpty()) {
				java.util.Iterator<AchoTaskWrapper> it = pending.iterator();
				while (it.hasNext()) {
					AchoTaskWrapper wrapper = it.next();
					this.remove(wrapper);
		...
		protected void afterExecute(Runnable r, Throwable t) {
			...
			AchoTaskWrapper wrapper = (AchoTaskWrapper) r;
			this._removeTaskFromDedupingSet(wrapper.getTask());
		 ...
		 * @param task
		 */
		private synchronized void _removeTaskFromDedupingSet(AchoTask task) {
			List<AchoTask> tasks = this.dedupingTaskList.get(task.getTaskId());

			if (tasks != null) {
				if (tasks.size() == 1) {
					this.dedupingTaskList.remove(task.getTaskId());
				} else {					
					Iterator<AchoTask> it = tasks.iterator();
					while (it.hasNext()) {
		...
		private class CleanupThread extends Thread {
			private final ReferenceQueue<AchoTaskCallback> referenceQueue;

			CleanupThread(ReferenceQueue<AchoTaskCallback> referenceQueue) {
			...
			public void run() {
				...
				while (true) {
					try {
						java.lang.ref.Reference<? extends AchoTaskCallback> toRemove = referenceQueue.remove();
